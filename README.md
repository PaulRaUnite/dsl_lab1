...
- **here recursy goes**: it performs appropriate class method from the non-deterministic automata class: `by_value` for leaf symbols, `by_concatenation` or `by_decision` for machines of its children and `by_clini` for only one child.
...
Transitions of DFA are stored as `Dict[int, Dict[chr, int]]`, there are no multiple transitions of the same symbol for one state, `0` is always start state, there can be multiple final states.
...
#### About algorithms of machine joining for NDFA
...
So, following rules are applied to join state machines to each other(`OR` is for `|` and `AND` for `+`(concatenation)):
...
The implementation of determinization uses [powerset construction technique](https://en.wikipedia.org/wiki/Powerset_construction). The article above is good, but uses epsilon moves which the implementation haven't, so keep it in mind.
...
The implementation uses fact, that if state `s1` equals to `s2` than if transition for `s1` by symbol `a` exists than transition for `s2` must exist and states to which they transfer must be equal, and vice versa.

For receiving equality sets, we build lower triangular matrix, where its indexes are states numbers(but without dublicates and diagonal(for memory use reduce) and because every state is equal to itself).

Algorithm:
1. For every `i` and `j` state, if one of them is final and other is not(XOR), we write 1(not equal, because only final states can have empty path to itself) to `ij` cell of the matrix.
2. Than, while we had changes in the previous iteration of the step, for every `i` and `j` that have zero in `ij` cell, we check all states that are reachable by the same vocabulary symbol of the automata, and if the reachable states are not equal each other, `i` and `j` is also not equal, so we put 1(not equal) to `ij` cell.
